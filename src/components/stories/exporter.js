...
export const blockToHTML = (block) => {
  ...
  switch (blockType) {
    ...
    case Block.CAPTION:
      return {
        start: `<p className="md-block-${blockType.toLowerCase()}"><caption>`,
        end: '</caption></p>',
      };
    case Block.IMAGE: {
      ...
      return {
        start: `<figure className="md-block-image${extraClass}"><img src="${imgData.src}" alt="${block.text}" /><figcaption className="md-block-image-caption">`,
    ...
    case Block.TODO: {
      ...
      return {
        start: `<div className="md-block-${blockType.toLowerCase()} ${containerClass}">${inp}<p>`,
        end: '</p></div>',
      };
    }
    case Block.BREAK:
